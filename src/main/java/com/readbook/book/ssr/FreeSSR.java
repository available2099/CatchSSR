...
import com.readbook.book.ssr.util.*;
...
/*              ExecutorService executorServiceCN2 = Executors.newFixedThreadPool(500);
        ...
        if (executorServiceCN2.isTerminated()) {
            ...
            createFile("isCN2", shuchu);
               ...
               ExecutorService executorServiceping = Executors.newFixedThreadPool(500);


               SSRNode  nodeListArryy [] = okNodeList.toArray(new SSRNode[]{});
               int quan = 0;
               for(int j = 0; j < (okNodeList.size()/10)+2; j++){
                   for (int i = 0; i < 10; i++) {
                       ...
                       executorServiceping.submit(
                               new Thread(new Runnable() {
                                   ...
                                   public void run() {
                                       ...
                                       try {
                                           System.out.println("ping " + nodeListArryy[Integer.valueOf(finalQuan *10+finalI)].getServer() + " :耗时:" + (System.currentTimeMillis() - startTime) * 1.0 / 1000 + " 秒");
                                           String result = null;
                                           try {
                                               result = getPingTime(nodeListArryy[Integer.valueOf(finalQuan *10+finalI)].getServer());
                                           } catch (IOException e) {
                                               e.printStackTrace();
                                           }
                                           if(!"请求超时".equals(result)){
                                               nodeListArryy[Integer.valueOf(finalQuan *10+finalI)].setAvgPingTime(result);
                                               okNodeList.add( nodeListArryy[Integer.valueOf(finalQuan *10+finalI)]);
                                           }
                                       }
